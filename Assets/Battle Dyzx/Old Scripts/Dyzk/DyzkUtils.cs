namespace BattleDyzx
{
    ...
    public struct DyzkAnalysis
    {
        ...
        public void AnalyzeTexture(Texture2D tex, string imageURI = "")
        {
            ...
            _pivot = new Vector2D(tex.width / 2.0f, tex.height / 2.0f);
            ...
            float radiusOfPrecision = Math.Max(_pivot.x, _pivot.y);
            _angleSpan = Math.PI * 2 / Math.Asin(1 / radiusOfPrecision);
            ...
            for (int i = 0; i < pixels.Length; i++)
            {
                ...
                {
                    Vector2D coords = new Vector2D(i % tex.width, i / tex.width);
                    _centerOfMass += coords;

                    PolarVector2D polarCoords =
                        PolarVector2D.FromCartesian(coords - _pivot);

                    int angIdx = (int)(polarCoords.a / (Math.PI * 2) * _angleSpan);
